...
def worker_process(
    ...
    print(f"Worker {process_id} (GPU {gpu_id}): Kicking off '{model_name}' for '{task_group_to_run}' (Batch: {batch_size})")
    ...
    try:
        ...
        print(f"DEBUG Worker {process_id}: Executing: {' '.join(command_to_run)}")
        result = subprocess.run(command_to_run, check=True, text=True, encoding='utf-8')

        print(f"Worker {process_id} (GPU {gpu_id}): Successfully completed '{task_group_to_run}'.")

    except subprocess.CalledProcessError as e:
        print(f"Worker {process_id} (GPU {gpu_id}): '{task_group_to_run}' failed with an error. Exit Code: {e.returncode}")
        print(f"Subprocess Output (if available):\n{e.stdout}")
        print(f"Subprocess Error (if available):\n{e.stderr}")
    except Exception as e:
        print(f"Worker {process_id} (GPU {gpu_id}): Unexpected error launching or monitoring evaluation for '{task_group_to_run}': {e}")
        import traceback
        traceback.print_exc()

def main():
    parser = argparse.ArgumentParser(description="LLM benchmark evaluations.")
    parser.add_argument("--num_gpus", type=int, help="Number of GPUs/workers to use. Default: all available or 1 for CPU.")
    parser.add_argument("--batch_size", type=int, default=1, help="Default batch size passed to worker tasks.")
    ...
    if not input_model_name:
        print("Model name cannot be empty. Exiting.")
        return
    ...
    if add_custom == 'yes':
        while True:
            ...
            if not all([custom_task_group, custom_bm_name, custom_module_path, custom_func_name]):
                ...
                if custom_task_group not in cust_ben_dety:
                    cust_ben_dety[custom_task_group] = {}
                cust_ben_dety[custom_task_group][custom_bm_name] = {
    ...
    if not chosen_initial_task_groups:
        print("No valid top-level task groups selected. Exiting.")
        return
    ...
    for task_group_name in chosen_initial_task_groups:
        ...
        if is_single_bm_task_group:
            ...
            if "skip" in selected_sub_indices_str or str(len(available_sub_benchmarks)+2) in selected_sub_indices_str:
                print(f"Skipping task group: {task_group_name}")
                continue
...
def display_consolidated_results(
    ...
    try:
        ...
        if model_df_display.empty:
            print(f"\nNo results for '{model_name}' in '{csv_path}'.")
            return
        ...
        for _, row in model_df_display.iterrows():
            task_bm_scores[row['Task']][row['Benchmark']] = row['Score']
        ...
        for task_group_name in ordered_task_groups_for_display:
            ...
            # Fix applied here: Calculate average if multiple benchmarks are selected,
            # or if it's a single-benchmark task group (where the single score is the average)
            if actual_scores_for_group_avg and (len(selected_bms_in_group) > 1 or is_single_bm_task_group):
                avg_score = sum(actual_scores_for_group_avg) / len(actual_scores_for_group_avg)
                current_model_row_data[(task_group_name, 'Average')] = round(avg_score, 2)
                multi_idx_cols.append((task_group_name, 'Average'))
            elif (len(selected_bms_in_group) > 1 or is_single_bm_task_group): 
        ...
        for col in multi_idx_cols:
            if col not in seen_cols:
                ...
                seen_cols.add(col)
        if not unique_cols:
            print("Warn: No columns generated for display table.")
            return
        ...
        def sort_key_final(col_tuple):
            ...
            try: 
                task_order = ordered_task_groups_for_display.index(tg_name)
            except ValueError:
                task_order = 9999
            is_special = tg_name in ["MMLU", "BBH", "AGIEval"]
            if is_special:
                sub_o = 0 if bm_name == '' else 1
                return (2, task_order, sub_o)
            else:
                if bm_name == 'Average':
                    bm_order = 99999 
                else:
        ...
        if not cols_to_sort_from_df and not df_disp.empty:
            cols_to_sort_from_df = df_disp.columns.tolist()
