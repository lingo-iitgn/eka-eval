...
def calculate_chrf_scores(predictions: List[str], references: List[str]) -> Dict[str, float]:
    """Calculate chrF++ scores for summarization evaluation"""
    # Filter valid predictions
    valid_pairs = []
    for pred, ref in zip(predictions, references):
        if (pred.strip() and ref.strip() and 
            pred not in ["[SUMMARIZATION ERROR]", "[INVALID_SUMMARY]"]):
            valid_pairs.append((pred, ref))
    
    if not valid_pairs:
        return {"chrF++": 0.0, "valid_samples": 0, "total_samples": len(predictions)}
    
    valid_predictions, valid_references = zip(*valid_pairs)
    
    # Prepare references in the format expected by sacrebleu (list of lists)
    ...
    try:
        ...
        chrf_score = corpus_chrf(
            valid_predictions,
            references_list,
            word_order=2,  # chrF++ uses word order = 2
            beta=2,        # chrF++ uses beta = 2
            remove_whitespace=True,
            char_order=6   # Default character n-gram order
        ).score
    except Exception as e:
...
def evaluate_crosssum_in(
    ...
    **kwargs
    ...
    for lang_code in target_languages:
        ...
        try:
            ...
            if not prompts:
                ...
                language_scores[lang_code] = {"chrF++": 0.0, "valid_samples": 0, "total_samples": 0}
                continue
            ...
            chrf_scores = calculate_chrf_scores(predictions, references)
            language_scores[lang_code] = chrf_scores

            # Add to overall scores
            if chrf_scores["chrF++"] > 0:
                all_chrf_scores.append(chrf_scores["chrF++"])

            detailed_results.extend(lang_detailed_results)

            logger.info(f"✅ {lang_code.upper()}: chrF++ = {chrf_scores['chrF++']:.2f} "
                       f"({chrf_scores['valid_samples']}/{chrf_scores['total_samples']} valid)")
    ...
    overall_scores = {
        "chrF++": np.mean(all_chrf_scores) if all_chrf_scores else 0.0,
        "languages_evaluated": len(all_chrf_scores),
    ...
    final_scores = {"CrossSum": overall_scores["chrF++"]}  # Use chrF++ as primary metric
    final_scores["CrossSum_chrF++"] = overall_scores["chrF++"]
    
    for lang, scores in language_scores.items():
        if scores and "chrF++" in scores:
            final_scores[f"CrossSum_{lang}"] = scores["chrF++"]

    logger.info(f"Overall CrossSum chrF++: {overall_scores['chrF++']:.2f}")
    return final_scores
...
if __name__ == '__main__':
    ...
    try:
        ...
        print("\n" + "="*70)
        print("🏆 CROSSSUM EVALUATION RESULTS (chrF++) 🏆")
        print("="*70)
        print(json.dumps(results, indent=2))
        
        # Print benchmark comparison
        print("\n📊 BENCHMARK COMPARISON:")
        print("Model Performance on CrossSum (chrF++):")
        print("• Gemma-2-2B:   6.57")
        print("• Llama-3.2-3B: 19.82") 
        print("• Llama-3.1-8B: 21.59")
        print("• Sarvam-1:     20.48")
        if "CrossSum" in results:
            print(f"• Your Model:   {results['CrossSum']:.2f}")
        print("="*70)
